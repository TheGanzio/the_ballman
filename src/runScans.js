...
import { getPostsDonnu, parseLinksDonnu} from './scans/parseDonnu.js'
import { getPostsATO, parseLinksATO} from './scans/parseATO.js'
import fs from 'fs'
...
const Donnu = mongoose.model('Donnu', PostsSchema)
const ATO = mongoose.model('ATO', PostsSchema)
...
function getData() {
  ...
  //     }
    ...
    // .then(links => {
    //     getPostsDonnu(links).then(posts => saveResultDonnu(JSON.stringify(posts, 0, 4))).catch(e => console.log(e))
    // })
    // .catch(e => console.log(e))

    parseLinksATO('https://www.ukrinform.ua/rubric-ato', '.othersBody a')
    .then(links => {
        getPostsATO(links).then(posts => saveResultATO(JSON.stringify(posts, 0, 4))).catch(e => console.log(e))
    })
    .catch(e => console.log(e))
